...
def get_mention_to_tokens_edges(num_mention, num_token, batch_entity_pos, batch_virtual_pos, batch_token_pos) -> Tuple[List[int], List[int]]:
    u = []
    v = []
        ...
        for entity_pos in entities_pos:
            for mention in entity_pos:
                ...
                for virtual_idx, _ in enumerate(virtual_pos):
                    ...
                    if virtual_right == mention[0] or mention[1] == virtual_left:
                        for token_idx, t_pos in enumerate(token_pos):

                            if virtual_left <= t_pos < virtual_right:
                                u.append(get_id(num_mention, batch_id, mention_idx))
