...
class GNN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        sent_type_embedding = self.node_type_embedding(torch.tensor(1).to(self.device)).view(1, 1, -1)
        virtual_type_embedding = self.node_type_embedding(torch.tensor(2).to(self.device)).view(1, 1, -1)
        ...
        node_hidden_state = torch.concat(
            (mention_hidden_state, sent_hidden_state, virtual_hidden_state, token_hidden_state), dim=1)
        ...
        mention_hidden_state = output_node_hidden_state[:, 0:num_mention]

        return mention_hidden_state
